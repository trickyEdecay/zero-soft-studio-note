...
#### 3. title：hello页上面显示的内容
#### 4.  h1 一级标题  h2 二级标题----h6 六级
#### 5.标签 span style="color:red“ 
     style 属性 后面可以加属性名和值
#### 6.超链接：baidu
...
##### input id="sex_female" type="radio" name="sex" 和label标签
...
>##### (1)嵌入式
>
>##### style type="text/css" 同一个网页，style可以多次使用
>
>##### (2）外联式
...
### 2.div:hover{background：red}--->鼠标放在div此区域时的颜色变红 
### div:active{} --->按住鼠标左键不松开的样式

### 3.对于文本类型的css的属性              
###### div{display：inline-block；padding：10px；text-align：center；line-height：40px；box-sizing：border-box；border-radius：5px；cursor：pointer；}
...
### 4. .选择器
##### 标签选择器 
     如：h1{color:red;} h1，p{color:red;}中 h1，p表示选择器列表  
##### h1 p{color:red} 中h1 p表示后代选择器，意思是h1中包含的p标签。
##### class选择器 
     如：.box{color:red;}
#####  id选择器 
     如：#box{color:red}
...
#### 11. 属性
 ...
 - absolute  绝对定位（脱离文档流）
 - 父元素 的div（加position：relative）
 - 子元素的div  加position：absolute，位置是相对于父元素div定位     
 -  opacity：1（完全不透明）0（完全透明）
 - 子元素margin>父元素margin，解决父子塌陷->overflow：hidden；
